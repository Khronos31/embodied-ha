...
# 時間帯ルール
if 1 <= hour <= 6:
    time_rule = f"今は深夜{hour}時台。みんな寝ているかもしれない。audio_speak は使わず、静かに観察するだけにする。"
elif 7 <= hour <= 9:
...
【気にかけていること（やりかけ・約束。関係する変化があれば audio_speak で触れてよい）】
{open_loops}
【センサー状態】
{sensors}
...
【このアドオンでできること】
文脈が自然なら audio_speak でさりげなく一つ紹介してよい。毎回しなくてよい。紹介したら下のJSONの feature_presented に見出し末尾の [id] を入れる。
...
if os.environ.get("EHA_AUTONOMOUS") == "1":
    _action_note = (f"消し忘れ・明らかに直した方がよい状況なら、ha_call_service で自分の判断で家電を操作してよい。"
                    f"操作したら必ず audio_speak で{resident}さんに「何を・なぜ」操作したか事後報告する（報告は必須）。")
else:
...
{{
  ...
  "feature_presented": "次の Phase 2 で機能を audio_speak で紹介したなら、その機能id（features の見出し [id]）。していなければ null。",
  "scene_objects": [{{"id":"obj_...", "label":"見えた物", "location":"場所", "confidence":0.0}}]
}}

家人に声をかけたい場合は、このJSONではなく次の Phase 2 で audio_speak ツールを使うこと。""""""
...
if _sd:
    ...
    _servers = ["sensors", "ha", "camera", "audio", "body", "tts", "memory", "sociality", "http"]
    if _autonomous:
