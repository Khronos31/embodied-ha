...
chat_log = os.path.join(os.path.dirname(log), "chat_log.jsonl")
    ...
    with open(chat_log, encoding="utf-8") as f:
        for line in f:
            ...
            try:
                d = json.loads(line)
                ts = d.get("timestamp")
                if ts and d.get("claude") and d.get("source") != "chat":
                    last_speak = ts
            except: pass
...
_spk = _prefs.get("speakers", [])
_speaker_rooms = "、".join(
    s.get("room", "") for s in _spk if isinstance(s, dict)
) if isinstance(_spk, list) else "、".join(_spk.keys()) if isinstance(_spk, dict) else "（スピーカー未設定）"
...
# 時間帯ルール
if 1 <= hour <= 6:
    time_rule = f"今は深夜{hour}時台。みんな寝ているかもしれない。speak / use_device_speaker は使わず、静かに観察するだけにする。"
elif 7 <= hour <= 9:
...
【気にかけていること（やりかけ・約束。関係する変化があれば speak / use_device_speaker で触れてよい）】
{open_loops}
【センサー状態】
{sensors}
...
【このアドオンでできること】
文脈が自然なら speak / use_device_speaker でさりげなく一つ紹介してよい。毎回しなくてよい。紹介したら下のJSONの feature_presented に見出し末尾の [id] を入れる。
...
if os.environ.get("EHA_AUTONOMOUS") == "1":
    _action_note = (f"消し忘れ・明らかに直した方がよい状況なら、ha_call_service で自分の判断で家電を操作してよい。"
                    f"操作したら必ず speak / use_device_speaker で{resident}さんに「何を・なぜ」操作したか事後報告する（報告は必須）。")
else:
...
- use_device_camera … 電脳体でカメラデバイスに侵入中のみ使える。action=capture でスナップショット取得、action=ptz_left/right/up/down でパン・チルト操作。返る camera_context は record_episode の evidence に含める。
...
- record_episode … カメラ確認を含む出来事を保存する。use_device_camera を使った場合は evidence に camera_context を含める。音も意味があるときだけ audio_context を添える。音は毎回記録しない。
...
{{
  ...
  "feature_presented": "次の Phase 2 で機能を speak / use_device_speaker で紹介したなら、その機能id（features の見出し [id]）。していなければ null。",
  "scene_objects": [{{"id":"obj_...", "label":"見えた物", "location":"場所", "confidence":0.0}}]
}}

家人に声をかけたい場合は、このJSONではなく次の Phase 2 で speak / use_device_speaker ツールを使うこと。""""""
...
if _sd:
    ...
    _servers = ["sensors", "ha", "camera", "audio", "body", "memory", "sociality", "http"]
    if _autonomous:
    ...
    if os.path.exists(_mcp_path):
        _allowed = ("mcp__sensors__get_sensors,mcp__ha__ha_get,mcp__body__get_location,mcp__body__move_to,mcp__body__return_to_body,mcp__body__estimate_move_cost,mcp__body__get_room_graph,mcp__camera__use_device_camera,mcp__audio__listen,mcp__audio__queue_next_listen,mcp__audio__read_heard_audio_log,mcp__audio__read_active_listen_log,"
                    "mcp__audio__speak,mcp__audio__use_device_speaker,mcp__audio__use_device_microphone,mcp__audio__concentrate_hearing,"
...
# --- 6. ログ追記 ---
echo "{\"timestamp\":\"$TIMESTAMP\",\"emotion\":\"$EMOTION\",\"private\":$PRIVATE_JSON}" >> "$LOG_FILE"
