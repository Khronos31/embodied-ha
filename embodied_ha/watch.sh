...
if [ "$LAST_DAYBOOK" != "$TODAY" ] && [ -f "$LOG_FILE" ] && [ -s "$LOG_FILE" ]; then
  ...
  LOG_FILE="$LOG_FILE" MEMORY_FILE="$MEMORY_FILE" TODAY="$TODAY" DAYBOOK_MARKER="$DAYBOOK_MARKER" LAST_DAYBOOK="$LAST_DAYBOOK" CHARACTER="$CHARACTER" RESIDENT="${RESIDENT:-ユーザー}" SCRIPT_DIR="$SCRIPT_DIR" python3 "$SCRIPT_DIR/daybook_rollup.py" || true
fi
