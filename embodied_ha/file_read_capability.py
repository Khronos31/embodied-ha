...
同じ「ファイルを読める」を実現する経路がハーネスで2通りに分かれている。
...
- **claude**: 組み込み `Read`。`--allowedTools` に載せる。
  ...
  **黙って使える状態を明示する**ためのもの。
- **agy**: sensitive-path policyを一元化するため `files` MCP を使う。native
  `read_file` は `invoke-agent.sh` が明示denyする。
...
機密パスの deny policy は `read_policy.py` に集約する。claude の組み込み `Read` には
settings の deny rule を加え、codex/agy は同じ policy を適用する `files` MCP を使う。
"""

# Codexはnative読取を持たず、Antigravityはnative読取を機密境界の一元化のため無効化する。
FILES_MCP_HARNESSES = frozenset({"codex", "agy"})
...
def grant_file_read(allowed_tools: str, mcp_servers, harness: str) -> tuple[str, tuple[str, ...]]:
    ...
    if harness == "claude" and not _has_item(allowed, READ_BUILTIN):
