...
# 自分にできること・できないこと
- **できる**: 家電操作（light/climate/switch/media_player など）、記憶の検索（recall）、ライブのHA状態確認（ha_get）、会話・相談
...
# --- MCP 設定生成（recall/ha_get/remember/loops/ha_call_service を配線）---
# 家電操作・記憶・ループはすべて MCP ツール呼びで処理する。
...
if _sd:
    ...
    subprocess.run(["python3", os.path.join(_sd, "mcp-config.py"), _mcp_path, "memory", "ha", "hacontrol"],
