...
claude CLI自体を呼ぶ箇所（invoke_claude）は薄いラッパーにして、
テストではモック化できるようにしてある。
"""
import json
import os
...
def build_message_envelope(prompt, prefix_blocks=None):
    """stream-json入力形式のユーザーメッセージ封筒を作る（chat.sh:512が基準、prefix_blocksは拡張）。

    prefix_blocksが与えられた場合、prompt本文のtextブロックの前に並べる
    （loop.shのobserveモードにおける投射カメラ画像ブロックの並び順
    ——画像/キャプション類が先、最後にユーザープロンプト本文——と同一）。
    chat.sh自体はcontent_blocksを使わず単一textブロックのみだったため、
    prefix_blocks省略時は従来通りの単一textブロックになる。
    """
    content = list(prefix_blocks) if prefix_blocks else []
...
def build_claude_command(
    *,
    ...
    mcp-config.pyをサブプロセスで呼びMCP設定ファイルを生成する副作用を
    伴う。run_mcp_configを差し替えることでテスト時にモック化できる。
    生成に失敗した/script_dirが空の場合、--allowedTools/--mcp-configは
    付かない（chat.shの既存の静かな劣化動作と同一）。
    """
    cmd = [
...
def invoke_claude(cmd, msg, cwd, env, run=subprocess.run):
    """claude CLIを実際に起動する薄いラッパー（chat.sh:564-566と同一）。
...
def log_tool_use_diagnostics(stdout, print_fn=None):
    """assistant側のtool_use呼び出しをstderrへ診断ログとして出す（chat.sh:574-580と同一、副作用のみ）。"""
    if print_fn is None:
        def print_fn(msg):
...
def extract_response_text(stdout, stderr, returncode, print_fn=None):
    """claude CLIのstdoutからRESPONSEに相当するテキストを取り出す（chat.sh:567-596と同一）。

    空応答の場合のみreturncode/stderr末尾を診断出力する。
    """
    if print_fn is None:
        def print_fn(msg):
            print(msg, file=sys.stderr)
    result_text = stream_result_payload(stdout)
    if not result_text.strip():
        print_fn(f"[chat][claude] 空応答 returncode={returncode}")
        if stderr.strip():
            print_fn(f"[chat][claude][stderr] {stderr.strip()[-400:]}")
    return result_text
