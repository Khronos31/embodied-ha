...
| ループ | 間隔 | 内容 |
|---|---|---|
| **観察** `watch` | 約 20 分 + センサートリガー | カメラ・センサーを確認し、気づき・感情・発話を生成 |
| **探索** `explore` | 約 30 分 | 自発的に家を調べる／内省する／Web 検索する |
| **会話** `chat` | オンデマンド | チャット入力に応答。家電操作・記憶検索もここから |
...
設定画面（⚙）からキャラクター・センサー・スピーカー・カメラ・ポリシーを編集できる。
...
| `preferences.json` | センサー・スピーカー・カメラ・エンティティ対応表 | Web UI 設定画面 or 会話 |
...
| `preferences.json` | センサー・スピーカー・カメラ設定 |
| `desires.json` | 欲求定義 |
