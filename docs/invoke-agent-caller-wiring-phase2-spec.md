...
1. **cwd統一(移行期間は二重export)**: `invoke-agent.sh`に全ハーネス共通`EHA_AGENT_CWD`を新設し、
   ...
   `run.sh`は移行期間中、`EHA_CLAUDE_CWD`と`EHA_AGENT_CWD`を**同じ値で両方export**する
   (後方互換シムではなく、増分7で旧経路コードを削除するのと同時にEHA_CLAUDE_CWDのexportも
   止める、という一体の移行として扱う)。`daybook_rollup.py`・ロールバック経路として温存される
   `loop.sh`/`chat.sh`は増分7完了まで`EHA_CLAUDE_CWD`のみを読み続けるため、この二重exportが
   外れる増分7時点で、これらが依然として正しいcwdを取得できることを確認してから完了とする。
...
7. **旧経路の削除**: `invoke_loop_claude()`の直接claude呼び出しコード、
   ...
   このタイミングで`run.sh`の二重export(増分1)も解消し、`EHA_CLAUDE_CWD`のexportを止める。
   `daybook_rollup.py`・`loop.sh`/`chat.sh`(ロールバック経路として温存)が
   引き続き正しいcwdを取得できることを最終確認してから完了とする。
8. **全体検証**: 全テストgreen、実CLIスモークテスト(observeの画像・queued listenのWAV
   それぞれ)、実機での本番相当smoke test。

## 受け入れ条件(ドラフト、増分確定時に精緻化)

- 全モード・chat.py両経路で、shadow parityテストが旧新一致を確認している。
- `loop.py`が`EHA_AGENT_HARNESS=agy`でSystemExitしない。
- `chat.py`のqueued listenが`--sound-file`を渡す。
- `--sound-file`を伴う呼び出しで`--allowed-builtins`が渡されていないことをテストで固定している。
- `EHA_AGENT_CWD`/`EHA_CLAUDE_CWD`解決結果のbyte-identical検証テストがgreen、かつ
  Claude組み込み自動メモリへの継続アクセスを実機確認済み。
