...
7. **旧経路の削除**: `invoke_loop_claude()`の直接claude呼び出しコード、
   `build_mcp_config()`/`--mcp-config`直渡しパスを削除。
   **訂正(2026-07-18、変更履歴参照)**: `chat.py`/`loop.py`の`EHA_CLAUDE_CWD`参照は
   当初「使われなくなった参照」として削除予定だったが、実装では
   `EHA_AGENT_CWD`優先・`EHA_CLAUDE_CWD`フォールバックのcwd解決として温存した
   (`chat.py`・`loop.py`のcwd解決部)。`run.sh`経由の本番では両変数が常に同値のため
   フォールバックは効かず、`run.sh`を経由しない直実行(開発環境・テスト)で
   旧変数のみが設定されている場合の後方互換として機能する。
...
- **訂正・削除**: 「全モード・chat.py両経路で、shadow parityテストが旧新一致を確認している」
  ...
  旧経路専用テスト削除により572→549件に減少(意図的、増分7完了時点の値。その後の
  増分8等でのテスト追加により2026-07-18現在は568件。
  [[embodied_ha_phase1_shadow_parity_tests_obsolete_2026-07-17]]参照)。
