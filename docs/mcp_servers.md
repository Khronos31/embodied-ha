...
音声の聴取、音声ログの読み取り、発話、デバイス侵入時のマイク/スピーカー操作、侵入不要の音声メディア受信を提供します。
...
- `concentrate_hearing` — 物理体にいるとき、その場で既定5秒（最大30秒）録音し、同じターンで
...
現在侵入中のカメラデバイスだけを操作するサーバーです。侵入不要の映像メディアは扱いません。

### ツール

- `use_device_camera` — `capture` でスナップショット取得、`ptz_left/right/up/down` で PTZ 操作
- `watch_media` — `video_media` を侵入なしで観る。文脈部屋を使うが、身体的侵入はしない

`camera_get` と `camera_ptz` という旧名は存在しません。`use_device_camera` は身体的知覚用、`watch_media` はメディア受信用です。メディア視聴は必要に応じて `record_episode(kind="media_watch")` で記憶に残せます。
...
`loop.py` の末尾で `daybook_rollup.py` がこれらをまとめて使います。
...
- `game_wordvec_race_start`（`mode="cpu"` でCPU戦を開始。別プロセスの `claude -p`(Haiku)を対戦相手にする）
- `game_wordvec_race_cpu_move`（CPU戦を1手進める。`game_over:true` になるまで繰り返す）
- `game_wordvec_race_submit`（人間戦。手番ごとに判定）
