...
## 2026-07-18: ドキュメント不整合3件の修正(PR#2最終レビュー指摘の消化)

PR#2最終レビュー(gpt-5.6-sol)で指摘されたドキュメント微不整合3件を修正した。
いずれも実装・テストは無変更、ドキュメントのみの修正。

1. **仕様書 増分7の`EHA_CLAUDE_CWD`削除記述と実装の乖離**: 増分7は
   「使われなくなった`EHA_CLAUDE_CWD`参照を削除」と記述していたが、実装は
   `chat.py`/`loop.py`のcwd解決を`EHA_AGENT_CWD`優先・`EHA_CLAUDE_CWD`
   フォールバックとして温存している。仕様書側を実装に合わせて訂正した。
   フォールバックが効くのは`run.sh`を経由しない直実行(開発環境・テスト)のみで、
   本番では二重exportにより両変数が常に同値。
2. **テスト件数549の時点差**: 受け入れ条件の「572→549件」は増分7完了時点の値であり、
   その後の増分8等でのテスト追加により現在は568件。時点の注記を追加した。
3. **`docs/loop-runtime-contracts.md`の「future loop.py」表記**: loop.pyカットオーバー
   完了(2026-07-16)後も読み書きインベントリ表の書き手欄が`future loop.py`のまま
   だったため、現行状態(loop.py=現行、loop.sh=ロールバック温存)へ更新した。
   あわせて同ドキュメントを日本語へ全面書き直し(docsの言語統一方針、README系は
   英語のまま例外)、shadow parity比較範囲の節は履歴として明示した。
